...
| -------- | ----- | ----------------------- | ------------------- | --------------- |
| `RUSTSEC-2020-0036` | `failure` | `bch_addr` / `cash_addr 0.1.0` used by BCH address code in `tcx-btc-kin` and `coin-bch` | No newer `bch_addr` release exists. Replacing the BCH address codec can change address compatibility. | Replace or vendor the BCH address codec with golden vectors for legacy and cashaddr behavior. |
...
| `RUSTSEC-2023-0037` | `xsalsa20poly1305` | `tcx-substrate` | Replacement `crypto_secretbox 0.2.0-pre.0` conflicts with the workspace RustCrypto `cipher 0.5.2` stack. | Plan a RustCrypto stack migration and verify substrate keystore compatibility. |
