...
impl AddressTrait for BtcKinAddress {
    fn from_public_key(
        ...
        let address = match seg_wit {
            ...
            constants::BTC_SEG_WIT_TYPE_VERSION_1 => BtcKinAddress {
                payload: BtcKinPayload::WitnessProgram {
                    version: WitnessVersion::V1,
                    program: wallet_core_common::btc::p2tr_program(&hex_to_bytes(public_key)?)?,
                },
...
impl BtcKinAddress {
    ...
    pub fn p2tr(network: &BtcKinNetwork, path: &str) -> Result<BtcKinAddress> {
        ...
        Ok(BtcKinAddress {
            payload: BtcKinPayload::WitnessProgram {
                version: WitnessVersion::V1,
                program: wallet_core_common::btc::p2tr_program(&hex_to_bytes(pub_key)?)?,
            },
            network: network.clone(),
        })
    }

    pub async fn p2tr_async<T>(
        transport: &T,
        network: &BtcKinNetwork,
        path: &str,
    ) -> Result<BtcKinAddress>
        ...
        T: AsyncApduTransport + ?Sized,
    {
        ...
        let apdu_res = transport
            .send_apdu(
                &BtcApdu::register_address(address.to_string().as_bytes())?,
                20,
            )
            .await?;
        ApduCheck::check_response(apdu_res.as_str())?;
        ...
        T: AsyncApduTransport + ?Sized,
    {
        ...
        let parent_xpub_data = get_xpub_data_async(transport, get_parent_path(path)?, true).await?;
