...
impl EosTransaction {
    ...
    pub async fn sign_tx_async<T: AsyncApduTransport>(
        transport: &T,
        tx_input: EosTxInput,
        sign_param: &SignParam,
    ) -> Result<EosTxOutput> {
        ...
        for sign_data in &tx_input.transactions {
            let mut eos_sign_result = EosSignResult {
                hash: String::new(),
                signs: vec![],
            };
            let tx_data_bytes = hex::decode(&sign_data.tx_hex)?;
            let tx_hash = sha256_hash(&tx_data_bytes).to_hex();
            eos_sign_result.hash = tx_hash;

            let mut tx_data_pack: Vec<u8> = Vec::new();
            tx_data_pack.put_slice(hex::decode(&sign_data.chain_id)?.as_slice());
            tx_data_pack.put_slice(hex::decode(&sign_data.tx_hex)?.as_slice());
            let context_free_actions = [0; 32];
            tx_data_pack.put_slice(&context_free_actions);
            let tx_data_hash = sha256_hash(&tx_data_pack);
            ...
            for pub_key in &sign_data.public_keys {
                let mut sign_data_pack: Vec<u8> = Vec::new();
                sign_data_pack.push(0x01);
                sign_data_pack.push(tx_data_hash.len() as u8);
                sign_data_pack.extend(tx_data_hash.iter());
                sign_data_pack.push(0x02);
                sign_data_pack.push(sign_param.path.len() as u8);
                sign_data_pack.extend(sign_param.path.as_bytes());
                sign_data_pack.extend(hex::decode(&view_info)?.as_slice());

                let bind_signature = {
                    let key_manager_obj = KEY_MANAGER.lock();
                    secp256k1_sign(&key_manager_obj.pri_key, &sign_data_pack)?
                };
                ...
                let prepare_apdus = EosApdu::prepare_sign(prepare_apdu_data);
                let mut prepare_result = String::new();
                for prepare_apdu in prepare_apdus {
                    prepare_result =
                        Self::send_checked(transport, prepare_apdu, constants::TIMEOUT_LONG)
                            .await?;
                }
                ...
                loop {
                    ...
                    let sign_result_vec =
                        Vec::from_hex(&sign_response[2..sign_response.len() - 6])?;
                    ...
                    let normalized_sig_vec = signature_obj.serialize_compact();
                    let sig_str = hex::encode(normalized_sig_vec.as_slice());
                ...
                let mut to_hash = hex::decode(&signature)?;
                to_hash.put_slice("K1".as_bytes());
                let signature_hash = ripemd160::Hash::hash(&to_hash);
                let check_sum = &signature_hash.as_byte_array()[0..4];

                let mut signature_slice = hex::decode(&signature)?;
                signature_slice.extend(check_sum);
                let signature_base58 =
                    "SIG_K1_".to_owned() + base58::encode(&signature_slice).as_ref();
                eos_sign_result.signs.push(signature_base58);

                trans_multi_signs.push(eos_sign_result.clone());
            }
        }

        Ok(EosTxOutput { trans_multi_signs })
    }

    pub async fn sign_message_async<T: AsyncApduTransport>(
        transport: &T,
        input: EosMessageInput,
        sign_param: &SignParam,
    ) -> Result<EosMessageOutput> {
        ...
        let mut prepare_response = String::new();
        for apdu in prepare_apdus {
            prepare_response = Self::send_checked(transport, apdu, constants::TIMEOUT_LONG).await?;
        }

        let pubkey = EosPubkey::pubkey_from_response(&prepare_response)?;
        let mut signature = String::new();
        if pubkey != input.pubkey {
            return Err(anyhow!("imkey_publickey_mismatch_with_path"));
        }
        ...
        loop {
            ...
            let sign_result_vec = Vec::from_hex(&sign_response[2..sign_response.len() - 6])?;
            let mut signature_obj = Signature::from_compact(sign_result_vec.as_slice())?;
            signature_obj.normalize_s();
            let signature_der = signature_obj.serialize_der().to_vec();
            let normalized_sig_vec = signature_obj.serialize_compact();
            ...
            if len_r == 32 && len_s == 32 {
                ...
                let pub_key_raw = hex::decode(&uncomprs_pubkey)?;
                let sign_compact = hex::decode(&sign_response[2..130])?;
                let rec_id = utility::retrieve_recid(&hash, &sign_compact, &pub_key_raw)?;
                let v = i32::from(rec_id) + 27 + 4;
