...
impl CosmosTransaction {
    ...
    pub async fn sign_async<T>(self, transport: &T) -> Result<CosmosTxOutput>
    where
        T: AsyncApduTransport + ?Sized,
    {
        check_path_validity(&self.path).unwrap();
        ...
        let mut prepare_data = {
            let key_manager_obj = KEY_MANAGER.lock();
            secp256k1_sign(&key_manager_obj.pri_key, sign_pack_vec.as_slice())?
        };
        prepare_data.insert(0, prepare_data.len() as u8);
        prepare_data.insert(0, 0x00);
        prepare_data.extend(sign_pack_vec.iter());
