use base64::Engine;
use bitcoin::bip32::Xpub;
use bitcoin::Network;
use num_bigint::BigInt;
use num_integer::Integer;
use num_traits::{FromPrimitive, Num, Zero};
use secp256k1::ecdsa::{RecoverableSignature, RecoveryId, Signature};
use secp256k1::{Message, PublicKey as PublicKey2, Secp256k1, SecretKey};
use std::convert::{TryFrom, TryInto};

pub fn hex_to_bytes(value: &str) -> anyhow::Result<Vec<u8>> {
    Ok(crate::hex::hex_to_bytes(value)?)
}

pub fn sha256_hash(data: &[u8]) -> Vec<u8> {
    crate::hash::sha256(data).to_vec()
}

pub fn secp256k1_sign(private_key: &[u8], message: &[u8]) -> anyhow::Result<Vec<u8>> {
    let message_hash = sha256_hash(sha256_hash(message).as_ref());
    let secret_key = SecretKey::from_byte_array(private_key.try_into()?)?;
    let message_data = Message::from_digest(message_hash.as_slice().try_into()?);
    let secp = Secp256k1::new();
    Ok(secp
        .sign_ecdsa(message_data, &secret_key)
        .serialize_der()
        .to_vec())
}

pub fn secp256k1_sign_verify(public: &[u8], signed: &[u8], message: &[u8]) -> anyhow::Result<bool> {
    let secp = Secp256k1::new();
    let public_obj = PublicKey2::from_slice(public)?;
    let hash_result = sha256_hash(message);
    let message_obj = Message::from_digest(hash_result.as_slice().try_into()?);
    let mut sig_obj = Signature::from_der(signed)?;
    sig_obj.normalize_s();
    Ok(secp
        .verify_ecdsa(message_obj, &sig_obj, &public_obj)
        .is_ok())
}

pub fn bigint_to_byte_vec(val: u64) -> Vec<u8> {
    crate::util::u64_to_be_bytes_vec(val)
}

pub fn uncompress_pubkey_2_compress(uncomprs_pubkey: &str) -> String {
    let x = &uncomprs_pubkey[2..66];
    let y = &uncomprs_pubkey[66..130];
    let y_bint = BigInt::from_str_radix(y, 16).unwrap();
    let two_bint = BigInt::from_i64(2).unwrap();

    let (_d, m) = y_bint.div_mod_floor(&two_bint);
    if m.is_zero() {
        ("02".to_owned() + x).to_lowercase()
    } else {
        ("03".to_owned() + x).to_lowercase()
    }
}

pub fn is_valid_hex(input: &str) -> bool {
    crate::hex::is_valid_hex(input)
}

pub fn retrieve_recid(
    msg: &[u8],
    sign_compact: &[u8],
    pubkey: &[u8],
) -> anyhow::Result<RecoveryId> {
    let secp_context = secp256k1::Secp256k1::new();

    let mut recid_final = -1i32;
    for i in 0..3 {
        let rec_id = RecoveryId::try_from(i)?;
        let sig = RecoverableSignature::from_compact(sign_compact, rec_id)?;
        let msg_to_sign = Message::from_digest(msg.try_into()?);

        if let Ok(rec_pubkey) = secp_context.recover_ecdsa(msg_to_sign, &sig) {
            let rec_pubkey_raw = rec_pubkey.serialize_uncompressed();
            if rec_pubkey_raw == pubkey {
                recid_final = i;
                break;
            }
        }
    }

    Ok(RecoveryId::try_from(recid_final)?)
}

pub fn to_ss58check_with_version(extended_key: Xpub, version: &[u8]) -> String {
    crate::bip32::xpub_to_ss58check_with_version(&extended_key, version)
}

pub fn from_ss58check_with_version(s: &str) -> anyhow::Result<(Xpub, Vec<u8>)> {
    crate::bip32::xpub_from_ss58check_with_version(s).map_err(|err| match err {
        crate::bip32::XpubError::InvalidBase58 => anyhow::anyhow!("invalid_base58"),
        err => anyhow::anyhow!(err),
    })
}

pub fn extended_pub_key_derive(extended_pub_key: &Xpub, path: &str) -> anyhow::Result<Xpub> {
    crate::bip32::derive_xpub(extended_pub_key, path).map_err(Into::into)
}

pub fn get_xpub_prefix(network: &str) -> Vec<u8> {
    crate::util::xpub_prefix_for_network_name(network).to_vec()
}

pub fn encrypt_xpub_with_key_iv(xpub: &str, key_hex: &str, iv_hex: &str) -> anyhow::Result<String> {
    let key_bytes = hex::decode(key_hex)?;
    let iv_bytes = hex::decode(iv_hex)?;
    let encrypted = crate::aes::cbc::encrypt_pkcs7(xpub.as_bytes(), &key_bytes, &iv_bytes)?;
    Ok(base64::engine::general_purpose::STANDARD.encode(encrypted))
}

pub fn network_convert(network: &str) -> Network {
    crate::bip32::bitcoin_network_from_name(network)
}

pub fn utf8_or_hex_to_bytes(value: &str) -> anyhow::Result<Vec<u8>> {
    crate::hex::utf8_or_hex_to_bytes(value)
}

pub fn version_at_least(version: &str, minimum: (u64, u64, u64)) -> bool {
    crate::util::version_at_least(version, minimum)
}

#[cfg(test)]
mod tests {
    use super::*;
    use hex::FromHex;

    #[test]
    fn hex_to_bytes_decodes_optional_prefix() {
        assert_eq!(
            vec![0x66, 0x6f, 0x6f, 0x62, 0x61, 0x72],
            hex_to_bytes("666f6f626172").unwrap_or_default(),
        );
        assert_eq!(
            vec![0x66, 0x6f, 0x6f, 0x62, 0x61, 0x72],
            hex_to_bytes("0x666f6f626172").unwrap_or_default()
        );
    }

    #[test]
    fn utf8_or_hex_to_bytes_matches_legacy_rules() {
        let tests = vec![
            ("0x1234", vec![0x12, 0x34]),
            ("1234", vec![0x31, 0x32, 0x33, 0x34]),
            ("0x1234abcd", vec![0x12, 0x34, 0xab, 0xcd]),
            (
                "1234abcd",
                vec![0x31, 0x32, 0x33, 0x34, 0x61, 0x62, 0x63, 0x64],
            ),
        ];

        for (input, expected) in tests {
            assert_eq!(utf8_or_hex_to_bytes(input).unwrap(), expected);
        }
    }

    #[test]
    fn version_at_least_uses_numeric_comparison() {
        assert!(!version_at_least("1.6.9", (1, 6, 11)));
        assert!(!version_at_least("1.6.10", (1, 6, 11)));
        assert!(version_at_least("1.6.11", (1, 6, 11)));
        assert!(version_at_least("1.7.0", (1, 6, 11)));
        assert!(version_at_least("2.0.0", (1, 6, 11)));
        assert!(!version_at_least("1.6", (1, 6, 11)));
        assert!(!version_at_least("1.6.beta", (1, 6, 11)));
    }

    #[test]
    fn sha256_hash_matches_vector() {
        let data = Vec::from_hex("11223344556677889900").unwrap();
        assert_eq!(
            hex::encode(sha256_hash(&data)),
            "6fa6810c930ba44a979a1bdb029f56cc608eafa043cea7e1ed21050d7456b5d3",
        );
    }

    #[test]
    fn secp256k1_signs_and_verifies() {
        let private_key =
            hex::decode("631e12677ef30f9b1a055b16bd9bf2d2a4f0795a484a9dc49683a05dc8328613")
                .unwrap();
        let public_key = hex::decode("04327a42790a3158d58bd68ee5763330b85b080c306534bf4d3c8fc711023db3090f302f9f7c8a2fc8ae81bfa22c9484b76326b1b2971eb7f7afea15cfd1996413").unwrap();
        let data = hex::decode("11223344556677889900").unwrap();
        let sign_result = secp256k1_sign(private_key.as_slice(), data.as_slice()).unwrap();
        assert_eq!(hex::encode(sign_result.clone()), "304402201b4197c869af37cea51e9ef34525c19f5e588ac5236b9e79dec3cdb1681498090220105d33d1217f76abd9a53ecab8beeb8de834ef5a5205a33288bb5bb4c3057742");
        let data = sha256_hash(data.as_slice());
        assert!(secp256k1_sign_verify(
            public_key.as_slice(),
            sign_result.as_slice(),
            data.as_slice()
        )
        .unwrap())
    }

    #[test]
    fn encodes_bigint_as_fixed_width_bytes() {
        assert_eq!(
            hex::encode(bigint_to_byte_vec(1111111111111111111)),
            "0f6b75ab2bc471c7"
        );
        assert_eq!(hex::encode(bigint_to_byte_vec(111111)), "000000000001b207");
    }

    #[test]
    fn compresses_uncompressed_pubkey() {
        let public_key_03 = "04327a42790a3158d58bd68ee5763330b85b080c306534bf4d3c8fc711023db3090f302f9f7c8a2fc8ae81bfa22c9484b76326b1b2971eb7f7afea15cfd1996413";
        assert_eq!(
            uncompress_pubkey_2_compress(public_key_03),
            "03327a42790a3158d58bd68ee5763330b85b080c306534bf4d3c8fc711023db309"
        );
        let public_key_02 = "04c390b4116d0f971c8f641f24346bd38377a22adb1426d27278e4cbb3e49e89986399de811e617faad763825e80af484e7fe16387929507baeaf633b03ce21f7e";
        assert_eq!(
            uncompress_pubkey_2_compress(public_key_02),
            "02c390b4116d0f971c8f641f24346bd38377a22adb1426d27278e4cbb3e49e8998"
        );
    }

    #[test]
    fn retrieves_recovery_id() {
        let msg = hex::decode("b998c88d8478e87e6dee727adecec067a3201da03ec8f8e8861c946559be6355")
            .unwrap();
        let sign_compact = hex::decode("73bcac6f18a619f047693afb17c1574fd22bb65d184888c13b5f2715304304b15919cbb66a8ae244ed8ac6dddbde8cc381a828961cfbad070d6c368941516ec5").unwrap();
        let pubkey = hex::decode("04aaf80e479aac0813b17950c390a16438b307aee9a814689d6706be4fb4a4e30a4d2a7f75ef43344fa80580b5b1fbf9f233c378d99d5adb5cac9ae86f562803e1").unwrap();
        assert!(retrieve_recid(msg.as_slice(), sign_compact.as_slice(), &pubkey).is_ok());
    }

    #[test]
    fn validates_hex() {
        assert!(is_valid_hex("666f6f626172"));
        assert!(!is_valid_hex("Hello imKey"));
        assert!(!is_valid_hex("+9qMMqskYEMjYyy0YnHtjXDRR62cpMDcoWFSA/"));
        assert!(!is_valid_hex(
            "R4Sod77GQJ1uxcQ8DH8giotqKTTiOQpt4ukd84VdKwdHUyeajiGhi6AUPSkyjsu1i0mXL"
        ));
        assert!(is_valid_hex(""));
    }

    #[test]
    fn converts_network_names() {
        assert_eq!(network_convert("MAINNET"), Network::Bitcoin);
        assert_eq!(network_convert("TESTNET"), Network::Testnet);
        assert_eq!(network_convert("mainnet"), Network::Bitcoin);
        assert_eq!(network_convert("ERRORNET"), Network::Testnet);
    }
}
