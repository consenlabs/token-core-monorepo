...
- **WHEN** 用 `sign_txs` 对一个含 N 个 item 的请求成功完成签名
...
- **WHEN** 通过 `sign_txs` 对 `[legacy_tx, eip1559_tx]` 进行批量签名
...
批量请求 SHALL 同时支持"整批共享同一 path"与"每笔指定独立 path"两种使用方式。每个 item 包含一个可选的 `path` 字段：当该字段为空字符串时，签名引擎 SHALL 使用外层 `path`（`tcx`：`SignTxsParam.path`；`ikc`：`SignParam.path`）；当该字段非空时，签名引擎 SHALL 使用该 item 自带的 `path` 进行派生与签名。同一批中允许部分 item 留空、部分 item 自带 `path`。
