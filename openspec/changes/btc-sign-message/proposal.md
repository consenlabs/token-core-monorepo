...
imkey-core 目前的现状与 token-core 一致，同样仅支持 BIP-322 Simple，需要与 token-core 并行实现上述扩展。Bitcoin Applet 已在 `>=1.6.11` 版本新增 message 签名专用接口（**单 INS `BTC_MSG_SIGN = 0x51`** + P2 分阶段协议，BIP32 path 嵌入 prepare TLV 并受 host ECDSA 签名覆盖），host 端通过 `get_btc_apple_version()` 做版本门控，低版本返回 `UpgradeApplet` 错误提示。BIP-322 相关的修改（输出格式变更和 Full 格式）可基于现有 PSBT 签名接口实现，无需 Applet 改动。

> **⚠️ ETH Applet 接口不可复用**：经 @xiaoguang1010 确认，BTC Standard/BIP-137 签名的 hash 方式（`Bitcoin Signed Message:\n` 前缀 + 双 SHA256）与 ETH 的 `personal_sign`（`\x19Ethereum Signed Message:\n` + Keccak-256）完全不同。虽然两者底层都使用 secp256k1 ECDSA，但 Applet 内部先算 hash 再签名，因此 ETH sign message 接口无法生成正确的 BTC 消息签名。BTC 侧最终以 Bitcoin Applet 自身的消息签名指令承载（host 预哈希 + applet 端 ECDSA + 用户屏幕确认），不复用 ETH 通道。
...
- **同步修改 imKey 硬件钱包路径**：BIP-137/Standard 走 Bitcoin Applet `>=1.6.11` 新增的**单 INS（`BTC_MSG_SIGN = 0x51`）+ P2 分阶段（`0x00` 暂存 / `0x80` 触发完整流程）**消息签名协议；host 端按 `[Signature TLV | Raw Data TLV(含 PRE_TAG_TXHASH / PRE_TAG_PATH 内层子 TLV)]` 打包，BIP32 path 嵌入 prepare TLV 并被 host ECDSA 签名覆盖，关闭 path 替换攻击面；末块 `P2=0x80` APDU 直接返回 66 字节 `len|R|S|V` 签名结果，host 完成 BIP-62 low-S 归一化与 flag byte 拼装后输出 Base64
...
- **imkey-core**：
    - `ikc-wallet/coin-bitcoin/src/message.rs` — 同步修改硬件钱包签名路径（新增 BIP-137 / Standard 格式支持、BIP-322 Full 格式支持）；BIP-137/Standard 流程按 applet 新协议组装"Signature TLV + Raw Data TLV（内含 hash/path 子 TLV）"，通过 `BtcApdu::btc_prepare(0x51, 0x00, …)` 单 INS 分块发送，末块 `P2=0x80` APDU 直接返回签名结果
    - `ikc-common/src/apdu.rs` — 移除历史的 `BtcApdu::btc_msg_sign(path)`（对应旧 INS `0x52`），不再发送独立的 sign APDU；INS `0x52` 在适配后保持为未分配值
...
Bitcoin Applet `>=1.6.11` 已交付消息签名指令（单 INS + path 嵌入 prepare TLV 的最终形态），与本提案的 host 端实现协同适配，可统一上线：

- token-core 全部签名格式（Standard / BIP-137 / BIP-322 Simple + Full）
- imkey-core BIP-322 编码修复（hex → Base64）+ BIP-322 Full 格式 + BIP-137 / Standard 通过 BTC applet 新指令完成
- imToken 客户端：软件钱包与 imKey 用户能力一致，覆盖所有 BTC 地址类型 × 兼容格式

历史曾考虑的"Phase 1 仅发布软件钱包 + Phase 2 等待 Applet"方案在 applet 协议落地后已不再必要，本次按一次性发布执行；imKey 用户在 BTC applet `<1.6.11` 时由 host 端拦截返回 `UpgradeApplet` 错误，引导用户升级 applet。
