...
> **⚠️ ETH Applet 接口不可复用**：经 @xiaoguang1010 确认，BTC Standard/BIP-137 签名的 hash 方式（`Bitcoin Signed Message:\n` 前缀 + 双 SHA256）与 ETH 的 `personal_sign`（`\x19Ethereum Signed Message:\n` + Keccak-256）完全不同。虽然两者底层都使用 secp256k1 ECDSA，但 Applet 内部先算 hash 再签名，因此 ETH sign message 接口无法生成正确的 BTC 消息签名。**BIP-137/Standard 格式对 imKey 的支持必须等待 Bitcoin Applet 固件升级。**
...
由于 imKey 的 BIP-137/Standard 支持依赖 Bitcoin Applet 固件升级，存在两种上线策略，需要产品侧对齐：

### 方案 A：分阶段上线
1. **Phase 1**（无 Applet 依赖，可立即发布）：
   - token-core 全部签名格式（Standard / BIP-137 / BIP-322 Simple + Full）
   - imkey-core BIP-322 编码修复（hex → Base64）+ BIP-322 Full 格式
   - imToken 客户端：软件钱包支持全部格式；imKey 用户仅支持 BIP-322（Native SegWit / Taproot）
2. **Phase 2**（等待 Applet 升级后）：
   - Bitcoin Applet 固件增加 message signing 专用接口
   - imkey-core 启用 BIP-137 / Standard 签名路径
   - imToken 客户端：imKey 用户也支持全部格式

### 方案 B：等待 Applet 就绪后统一上线
- 所有功能（token-core + imkey-core + Applet）一起发布，imKey 和软件钱包从第一天起能力一致

### 建议
**推荐方案 A**。BIP-322（SegWit / Taproot）已覆盖当前主流地址类型的签名需求，Phase 1 即可满足大部分用户场景。Legacy 和 Nested SegWit 地址的 BIP-137/Standard 签名对 imKey 用户的需求优先级较低，可在 Applet 升级就绪后补齐。
